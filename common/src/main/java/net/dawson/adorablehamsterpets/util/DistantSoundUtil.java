...
public final class DistantSoundUtil {
    ...
    public static void stopSession(ServerWorld world, UUID sessionId) {
        ModPackets.StopDistantSoundS2CPacket packet = new ModPackets.StopDistantSoundS2CPacket(sessionId);
    ...
    private static void broadcastStart(
        ...
        for (ServerPlayerEntity player : world.getPlayers()) {
            ...
            if (volume > MINIMUM_VOLUME) {
                ...
                ModPackets.PlayDistantSoundS2CPacket packet = sessionKey.isEmpty()
                        ? ModPackets.PlayDistantSoundS2CPacket.positioned(
                                sound.getId(),
                                sourcePosition,
                                baseVolume,
                                pitch,
                                (float) audioRange,
                                category)
                        : ModPackets.PlayDistantSoundS2CPacket.startSession(
                                sound.getId(),
                                sourcePosition,
                                sourceEntityId,
                                baseVolume,
